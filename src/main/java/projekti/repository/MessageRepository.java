...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query(value = "SELECT * FROM MESSAGE WHERE ID IN (SELECT ID FROM MESSAGE LEFT JOIN FOLLOWING ON (USERNAME = ACCOUNT_ID) WHERE FOLLOWING_ID = ?1 ORDER BY DATETIME DESC LIMIT 25)\n"
            + "UNION\n"
